...
time_data <- 
  ...
  filter(!state %in% c("American Samoa","Guam","Northern Mariana Islands","Puerto Rico","Virgin Islands","Diamond Princess","Grand Princess"))
...
ggplot(state_longer_elections, aes(daycount,scaled_case, color = factor(party, c("republican","democrat")), group = state )) +
  geom_line() +
...
#Graphs subset
ggplot(state_longer_rep, aes(daycount,scaled_case, color = state, group = state )) +
  geom_line() +
  theme(legend.title = element_blank()) +
  labs(
...
#Graphs Subset
ggplot(state_longer_dem, aes(daycount,scaled_case, color = state, group = state )) +
  geom_line() +
  theme(legend.title = element_blank()) +
  labs(
